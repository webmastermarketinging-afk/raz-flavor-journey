...
const StrainShowcase = () => {
  ...
  const currentStrain = strains[activeStrain];
  return <section className="relative py-20 overflow-hidden no-hover-anim" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex justify-center mb-12">
          <div className="flex gap-4 p-2 glass rounded-full">
            {strains.map((strain, index) => <button key={strain.id} onClick={() => setActiveStrain(index)} className={`px-6 py-3 rounded-full font-poppins-bold text-sm ${activeStrain === index ? `${strain.gradient} text-white shadow-neon` : 'text-muted-foreground'}`}>
                {strain.type}
              </button>)}
          </div>
        </div>
        ...
        <div className="strain-showcase">
          <div className="grid lg:grid-cols-2 gap-12 items-center max-w-6xl mx-auto">
              ...
              <div className="strain-item">
                <div className="product-card p-8 text-center">
                  <div className="relative mb-6">
                    <img src={currentStrain.images[activeImageIndex]} alt={currentStrain.name} className="w-full max-w-sm mx-auto rounded-xl" />
                    <div className={`absolute -inset-4 ${currentStrain.gradient} opacity-20 rounded-xl blur-xl`} />
                    
                    {/* Navigation arrows for images */}
                    {currentStrain.images.length > 1 && <>
                        <button onClick={prevImage} className="absolute left-2 top-1/2 transform -translate-y-1/2 w-8 h-8 rounded-full glass border border-white/10 text-foreground">
                          <ChevronLeft className="w-4 h-4 mx-auto" />
                        </button>
                        <button onClick={nextImage} className="absolute right-2 top-1/2 transform -translate-y-1/2 w-8 h-8 rounded-full glass border border-white/10 text-foreground">
            ...
            <div className="space-y-8">
              <div>
                <h3 className="font-druk text-4xl text-foreground mb-4">Strain Type</h3>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  {currentStrain.effects.map((effect, index) => <span key={index} className="px-4 py-2 glass rounded-full text-foreground font-poppins-bold text-sm">
                      {effect}
                    </span>)}
                </div>
              </div>
              ...
              <div>
                <h3 className="font-druk text-4xl text-foreground mb-4">Strain Main</h3>
                <p className="text-lg text-muted-foreground leading-relaxed">
                  {currentStrain.description}
                </p>
              </div>

              <button className={`px-8 py-4 ${currentStrain.gradient} text-white font-poppins-bold text-lg rounded-2xl`} onClick={() => navigate('/contact')}>
        ...
        <div className="flex justify-center mt-12 gap-2">
          {strains.map((_, index) => <div key={index} className={`w-3 h-3 rounded-full ${activeStrain === index ? `${strains[index].gradient} shadow-neon` : 'bg-muted'}`} />)}
        </div>
      </div>
    </section>;
};
